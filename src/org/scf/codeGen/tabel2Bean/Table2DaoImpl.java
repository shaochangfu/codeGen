...
public class Table2DaoImpl {
	...
	public static void main(String[] args) throws Exception {
		Table2DaoImpl maker = new Table2DaoImpl();
		maker.init();
		File targetFilepath = new File(targetPath);//+"\\"+tableStr.toLowerCase()
		if(!targetFilepath.exists()){
			targetFilepath.mkdirs();
		}
		String templateFile = "DaoImpl.ftl";// 模板文件
		...
		if(map != null){
			...
			while(it.hasNext()){
				...
				File targetFile = new File(targetFilepath.getPath()+"\\"+firstUpper(key)+"DaoImpl.java");
