...
public class Table2BeanUtil {
	...
	private String url = "jdbc:mysql://localhost:3306/achangfuprd?useUnicode=true&amp;characterEncoding=UTF-8";
	private static Connection con = null;
	private static String targetPath = "src\\org\\scf\\common\\bean";
	private static String packagePath = "org.scf.common.bean";
	...
	public static void main(String[] args) throws Exception {
		...
		Map<String,Map> map = JDBCUtil.getTables(con,packagePath);
		if(map != null){
			Iterator<String> it = map.keySet().iterator();
			while(it.hasNext()){
				String key = it.next();
				Map columnMap = map.get(key);
				File targetFile = new File(targetFilepath.getPath()+"\\"+firstUpper(key)+".java");
