...
public class Table2MyBatisMapper {
	...
	public static void main(String[] args) throws Exception {
		Table2MyBatisMapper maker = new Table2MyBatisMapper();
		maker.init();
		File targetFilepath = new File(targetPath);//+"\\"+tableStr.toLowerCase()
		if(!targetFilepath.exists()){
			targetFilepath.mkdirs();
		}
		String templateFile = "mybatisMapper.ftl";// 模板文件
		...
		if(map != null){
			...
			while(it.hasNext()){
				...
				File targetFile = new File(targetFilepath.getPath()+"\\"+firstUpper(key)+".xml");
