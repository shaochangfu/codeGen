...
public class Table2ServiceImpl {
	...
	public static void main(String[] args) throws Exception {
		Table2ServiceImpl maker = new Table2ServiceImpl();
		maker.init();
		File targetFilepath = new File(targetPath);//+"\\"+tableStr.toLowerCase()
		if(!targetFilepath.exists()){
			targetFilepath.mkdirs();
		}
		String templateFile = "ServiceImpl.ftl";// 模板文件
		...
		if(map != null){
			...
			while(it.hasNext()){
				...
				File targetFile = new File(targetFilepath.getPath()+"\\"+firstUpper(key)+"ServiceImpl.java");
