...
public class Table2Controler {
	...
	public static void main(String[] args) throws Exception {
		Table2Controler maker = new Table2Controler();
		maker.init();
		File targetFilepath = new File(targetPath);//+"\\"+tableStr.toLowerCase()
		if(!targetFilepath.exists()){
			targetFilepath.mkdirs();
		}
		String templateFile = "Controller.ftl";// 模板文件
		...
		if(map != null){
			...
			while(it.hasNext()){
				...
				File targetFile = new File(targetFilepath.getPath()+"\\"+firstUpper(key)+"Controller.java");
